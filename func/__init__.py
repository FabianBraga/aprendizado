...
#Função que testa uma condição e retorna um valor que pode ser lógico ou string
def teste(texto=f'Quer continuar [S/N]:', opcoes='SN', retorno=True):
    """
    -> esta função vai testar uma condição passada pelo parametro opcoes
        :param texto: O texto a ser apresentado na pergunta
        :param opcoes: Um conjunto de caracteres a ser avaliado devem ser passados em Maiusculas
        :param retorno: se True vai retornar um valor lógico se False vai retornar a Letra escolhida

        :return: retorna uma letra se retorno for False e um valor boleano se for True sendo que para esse ultimo,
                 só vão ser avaliadas as duas primeiras letras considerando a primeira True e a segunda False
    """
    if retorno:
        opcoes = opcoes[:2]
    while True:
        try:
            test = str(input(texto)).upper()[0]
            if test in opcoes:
                break
            else:
                print('Resposta fora da lista de opções da sentença')
        except:
            print('Deve ser informado um valor válido, tente novamente.')
            continue
    if retorno:
        if test == opcoes[0]:
            ret = True
